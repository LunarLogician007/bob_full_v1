...
bitgen.py - bob FASM <-> configuration chain, and the .bit file (M10).
  ...
  tools/bob/bitgen.py design.fasm -o design.bit [--bram 0:contents.json] [--meta k=v]
  tools/bob/bitgen.py --dump design.bit            the chain back as FASM (stdout)
  tools/bob/bitgen.py --roundtrip design.bit       bits -> FASM -> bits must be identical

FASM (written by tools/bob/fasm_from_vpr.py, read here): one feature per line,
...
it was built) and a CRC over the whole file. host/bob load writes the chain through
