...
run.py - Python pack/place/route of an example (M12a).
  ...
  tools/bob/pnr/run.py counter [--seed N] [--pcf pins.pcf] [--name NAME]   -> build/pnr/<name>/

Same input as VPR (tools/bob/vpr_run.prepare: the eblif and the fixed pins), same
output files as VPR (.net .place .route + <name>.vpr.json), so
tools/bob/fasm_from_vpr.py turns either into FASM and chain bits. No Docker.
