...
equiv.py - the synthesised netlist equals its source Verilog (M8), by simulation.
  ...
  tools/bob/equiv.py examples/counter.v [--top counter] [--cycles 300]

Both the source and <top>_syn.v (tools/bob/synth.py, simulated with
tools/bob/synth/bob_cells_sim.v, whose BRAM/DSP models are the fabric's own
...
[pad_i, LEDs just before the edge, LEDs just after it]. tools/bob/place.py (model),
hw/tb/tb_synth.v (fabric RTL) and host/hwtest.py (the board) check against it.
