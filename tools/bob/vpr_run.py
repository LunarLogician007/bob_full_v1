...
vpr_run.py - pack, place and route a synthesised bob netlist with VPR (M9).
  ...
  tools/bob/vpr_run.py counter [--seed N]        -> build/vpr/<top>/

Input is tools/bob/synth.py's build/synth/<top>/<top>.json. VPR (OpenFPGA's Docker
image, the same one that built the rr graph) runs on tools/bob/arch/bob_k<K>.xml
...
RESULTS = os.path.join(HERE, "vpr")                  # committed results: tools/bob/vpr/<top>/
...
def write_eblif(top, mod, pin_map=None):
    ...
    L.append(f"# GENERATED by tools/bob/vpr_run.py from build/synth/{top}/{top}.json")
...
EXAMPLES = ["gates", "adder", "counter", "blinky", "ram", "mult", "switches", "fir", "wide"]
