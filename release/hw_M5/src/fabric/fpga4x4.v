...
module fpga4x4 #(
    parameter [31:0] IDCODE_VALUE   = 32'h7BEEF093,
    parameter [31:0] USERCODE_VALUE = 32'h00000005,
    ...
    localparam integer CHAIN_W = CTRL_W + FAB_W + BRAM_W;

    // TAP
    wire                bsr_capture, bsr_shift, bsr_update, bsr_mode;
    wire                bsr_si, bsr_so, tlr;
    wire                dr_capture, dr_shift, dr_update;
    wire                sel_cfg_in, sel_cfg_out, sel_ctrl, sel_capture, sel_bram;
    wire                cfg_so, ctrl_so, cap_so, bram_so;
    ...
    wire                fab_so, bram_chain_so;
    wire [FAB_W-1:0]    cfg_out;
    wire [CTRL_W-1:0]   ctrl_cfg;
    wire [BRAM_W-1:0]   bram_cfg;
    wire [CHAIN_W-1:0]  chain_cfg = {bram_cfg, cfg_out, ctrl_cfg};   // chain bit k = chain_cfg[k]
    ...
    // TDI -> BRAM tile -> fabric tiles (top first) -> ctrl tile -> TDO
