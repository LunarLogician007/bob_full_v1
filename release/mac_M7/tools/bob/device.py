...
Every consumer reads what this file generates instead of repeating numbers:
  ...
  tools/bob/arch/bob_k6.xml         VPR architecture (and bob_k4.xml)
  tools/bob/device.json             host tools (host/bitstream.py, model.py, bitgen)
...
--------------------------------
  1. this file describes the grid, the tiles' pins and the routing architecture
     and writes a VPR architecture (tools/bob/vpr_arch.py, derived from
  ...
  2. VPR builds its tileable routing-resource graph and routes a trivial
     netlist on it (tools/bob/vpr_rrgraph.sh, Docker; `make rrgraph`)
  ...
  4. tools/bob/fabric_gen.py writes the fabric RTL from the same data
...
class Device:
    ...
    def _grid(self):
        ...
        self.board_inputs = [{"name": n, "pad": pad_of[(0, 1 + i)], "bit": i}
                             for i, n in enumerate(BOARD_INPUTS)]
        self.board_outputs = [{"name": n, "pad": pad_of[(W - 1, 1 + i)], "bit": i}
    ...
    def to_json(self):
        ...
        return {
            ...
            "arch": {"grid_width": self.width, "grid_height": self.height,
                     ...
                     "vpr_arch": f"tools/bob/arch/bob_k{self.lut_k}.xml",
                     "rr_graph": f"tools/bob/arch/bob_k{self.lut_k}_rr.xml.gz",
