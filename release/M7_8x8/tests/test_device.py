...
@pytest.mark.parametrize("k,chain_w", [(6, 9400), (4, 6808)])
def test_sizes(k, chain_w):
    """The M7 device, pinned: a change here must be a deliberate one."""
    dev = DEVICES[k]
    assert (dev.width, dev.height, dev.arch["chan_width"]) == (10, 10, 24)
    assert {t: len(v) for t, v in dev.by_type.items()} == {"io": 32, "clb": 48, "bram": 2, "dsp": 2}
...
def test_board_pads():
    ...
    for p in dev.board_inputs:
        assert dev.pads[p["pad"]].x == 0, "board inputs sit on the West edge"
...
def test_generated_fabric_has_every_mux_and_block():
    ...
    assert len(re.findall(r"^\s*clb u_clb_x\d+y\d+ ", text, re.M)) == 48
...
def test_bitstream_py_constants_come_from_device_json():
    ...
    assert (B.LUT_K, B.CLB_CFG_W, B.NCLB, B.NBRAM, B.NDSP, B.NPAD, B.BSR_W) == (6, 71, 48, 2, 2, 32, 64)
...
def test_bitstream_engine_at_k4(tmp_path):
    ...
    code = (
        ...
        "assert B.LUT_K == 4 and B.FABRIC_CFG_W == 6808\n"
