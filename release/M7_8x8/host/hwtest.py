...
def check_counter8(p, ctx):
    ...
    import fpga
    from designs import d_counter
    if not fpga.load_bitstream(p, d_counter("jtag", x=8, bits=8).build(), verbose=False):
        return False, "load failed"
    n = _capture_w()
    cfgplane.user1(p, 0b101)
    cfgplane.ir(p, "CAPTURE")
    vals = [_counter_value(p.shift_dr(n, 0), x=8, bits=8) for _ in range(12)]
    cfgplane.user1(p, 0)
    diffs = [(b - a) % 256 for a, b in zip(vals, vals[1:])]
    return all(d == n + 5 for d in diffs), f"counts {vals} (steps {diffs}, want {n + 5} each)"
