...
place.py - pack and place a synthesised bob netlist onto the fabric (M8).
  ...
  tools/bob/place.py counter [--check] [--seed N]
...
routing at M9. It reads build/synth/<top>/<top>.json (tools/bob/synth.py), builds
a host/bitstream.py Design, and lets that router route it.
...
--check replays <top>.trace.json (tools/bob/equiv.py: the SOURCE Verilog's outputs
under random inputs) through tools/bob/model.py on the placed bitstream.
"""
...
def flow(top, files=None, cycles=300):
    ...
    import equiv
    files = files or [os.path.join(ROOT, "examples", f"{top}.v")]
