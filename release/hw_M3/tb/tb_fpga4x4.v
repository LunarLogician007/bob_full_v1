// -----------------------------------------------------------------------------
// tb_fpga4x4.v - self-checking testbench for the 4x4 fabric (M3)
//
...
// host/bitstream.py, expected outputs from its software model and CRCs from
// tools/bob/chainbits.py. A disagreement shows up as a named failing vector.
//
...
module tb_fpga4x4;
    ...
    localparam        HALF     = 50;
    localparam [31:0] IDCODE   = 32'h5BEEF093;
    localparam [31:0] USERCODE = 32'h00000003;
    localparam integer CFG_W   = 2896;
    ...
    fpga4x4 #(.IDCODE_VALUE(IDCODE), .USERCODE_VALUE(USERCODE)) dut (
        .tck(tck), .tms(tms), .tdi(tdi), .tdo(tdo),
        .pad_i(pad_i), .pad_o(pad_o),
        .tap_state(tap_state), .configured(configured)
    );

    wire [CFG_W-1:0] cfg = dut.cfg_out;

    // bitstreams, CRCs, run_designs and check_showcase
    `include "vectors.vh"
    ...
    initial begin
        ...
        $display("");
        $display("=== 4x4 CLB fabric on the scan-chain configuration plane (M3) ===");
        $display("    config chain: %0d bits, 16 tiles x 181", CFG_W);
        ...
        shift_ir(IR_CFG_OUT, irc);
        shift_marker(tail);
        check("marker emerges after exactly 2896 bits", {32'h0, tail}, {32'h0, MARKER});
        ...
        shift_ir(IR_USER1, irc);
        shift_dr(32, 128'h1, rx);          // ce = 1
        idle(8);
        shift_ir(IR_CAPTURE, irc);
        shift_dr(16, 128'h0, rx);
        check("under GSR, tile(0,0) q = INIT (1)", {63'h0, rx[0]}, 64'h1);
        check("GTS: pad_o[0] forced 0 even though q = 1", {63'h0, pad_o[0]}, 64'h0);
        // Step startup one TCK at a time. GSR drops on edge 1 and GWE rises on
        // edge 3, so edges 2 and 3 see GSR=0 with GWE=0: the flip-flop must stay
        // frozen at INIT there, and only clock on the first edge with GWE=1.
        // (Without this window a fabric that ignores GWE passed every test -
        // found by sim/mutate_fabric.sh.)
        shift_ir(IR_JSTART, irc);
        tick(1'b0, 1'b0);
        ...
        tick(1'b0, 1'b0);
        tick(1'b0, 1'b0);
        check("edges 2-3 had GSR=0, GWE=0: q frozen at INIT (1)", {63'h0, dut.clb_o[0]}, 64'h1);
        check("GWE now set", {63'h0, dut.u_ctrl.gwe}, 64'h1);
        tick(1'b0, 1'b0);
        check("first edge with GWE=1: q clocked to D (0)", {63'h0, dut.clb_o[0]}, 64'h0);
        check("DONE (LD3) on after edge 4", {63'h0, configured}, 64'h1);
        idle(8);
        shift_ir(IR_CAPTURE, irc);
        shift_dr(16, 128'h0, rx);
        check("after JSTART (GSR off, GWE on), q clocked to D (0)", {63'h0, rx[0]}, 64'h0);
        shift_ir(IR_USER1, irc);
        shift_dr(32, 128'h0, rx);
        ...
        $display("");
        $display("[9] reset restores IDCODE");
        goto_reset_then_idle;
        shift_dr(32, 128'h0, rx);
