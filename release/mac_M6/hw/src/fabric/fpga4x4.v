...
module fpga4x4 #(
    ...
    parameter [31:0] USERCODE_VALUE = 32'h00000006,
    ...
    parameter integer BRAM_W        = `BOB_BRAM_W,
    parameter integer DSP_W         = `BOB_DSP_W,
    ...
    localparam integer CHAIN_W = CTRL_W + FAB_W + BRAM_W + DSP_W;
    ...
    wire                fab_so, bram_chain_so, dsp_chain_so;
    wire [FAB_W-1:0]    cfg_out;
    wire [CTRL_W-1:0]   ctrl_cfg;
    wire [BRAM_W-1:0]   bram_cfg;
    wire [DSP_W-1:0]    dsp_cfg;
    wire [CHAIN_W-1:0]  chain_cfg = {dsp_cfg, bram_cfg, cfg_out, ctrl_cfg};   // chain bit k = chain_cfg[k]
    wire [15:0]         bram_trk;                                             // BRAM -> fabric East edge
    wire [15:0]         dsp_trk;                                              // DSP  -> fabric North edge
    ...
    // TDI -> DSP tile -> BRAM tile -> fabric tiles (top first) -> ctrl tile -> TDO
    cfg_tile_sr #(.W(DSP_W)) u_dsp_cfg (
        .tck     (tck),
        .capture (cfg_capture),
        .shift   (cfg_shift),
        .commit  (cfg_commit),
    ...
    cfg_tile_sr #(.W(BRAM_W)) u_bram_cfg (
        .tck     (tck),
        .capture (cfg_capture),
        .shift   (cfg_shift),
        .commit  (cfg_commit),
    ...
    assign north_in = dsp_trk;               // M6: the DSP tile drives the North edge

    // ---------------------------------------------------------------------
    // M6: the DSP tile on the North edge (hw/src/tiles/dsp_tile.v)
    // ---------------------------------------------------------------------
    dsp_tile u_dsp (
        .sysclk     (sysclk),
        .gce        (gce),
    ...
    assign east_in  = bram_trk;              // M5: the BRAM tile drives the East edge

    // ---------------------------------------------------------------------
    // M5: the BRAM tile on the East edge (hw/src/tiles/bram_tile.v)
    // ---------------------------------------------------------------------
    bram_tile u_bram (
        .sysclk     (sysclk),
        .gce        (gce),
