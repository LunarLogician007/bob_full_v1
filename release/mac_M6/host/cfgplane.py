...
BRAM_CMD = {"nop": 0, "load_ptr": 1, "write": 2, "read": 3, "drive": 4}
...
def bram_scan(p, cmd="nop", payload=0):
    ...
    if st["version"] != 0x05:
        raise RuntimeError(f"USER4 version 0x{st['version']:02X}: no BRAM tile in this bitstream")
    return st
...
def dsp_scan(p, drive=None):
    ...
    if st["version"] != 0x06:
        raise RuntimeError(f"DSP register version 0x{st['version']:02X}: no DSP tile in this bitstream")
    return st
