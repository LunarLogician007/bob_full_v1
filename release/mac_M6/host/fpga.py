#!/usr/bin/env python3
"""
Compile a design, generate its bitstream, load it into the 4x4 fabric over JTAG,
and run it on the board.

  ./fpga.py --list                 what designs are built in
  ./fpga.py --report three         place and route it, print the result, no hardware
  ./fpga.py --load and             build it, load it, then watch the switches
  ./fpga.py --selftest             load every design and verify it through INTEST
  ./fpga.py --watch                monitor the boundary, leave the config alone
  ./fpga.py --bscan                drive the fabric from JTAG: toggle the
                                   boundary cells by hand and watch the result
  ./fpga.py --load xor --bscan     load a design, then take the switches over

The fabric's pads: SW1/SW0 and BTN3..0 are pad_i[5:0]; LD0/LD1/LD2 are
pad_o[0..2]; LD3 is DONE (configured, CRC checked, startup complete).

Since M3 the fabric sits behind the scan-chain configuration plane with the
6-bit AMD instruction set; loading goes through host/cfgplane.py
(docs/bitstream-format.md section 8).
"""

import argparse
import sys
import time

import bscan
import buildcfg
import cfgplane
from dirtyjtag import Probe
from bitstream import FABRIC_CFG_W, simulate
from designs import DESIGNS, BY_KEY

_CFG = buildcfg.read_cfg()
IDCODE_FABRIC = (buildcfg.expected_idcode(_CFG)
                 if _CFG.get("top") == "fpga4x4_top" and _CFG.get("idcode") else 0x6BEEF093)
CSI = "\x1b["


# --- configuration ---------------------------------------------------------

def load_bitstream(p, bs, verbose=True, force_slow=False):
    """Load the 2896-bit chain through the configuration plane and start it:
    JPROGRAM, expected CRC, CFG_IN, status, CFG_OUT readback, JSTART, DONE.

    The readback is not optional: it is the only thing standing between a
    silently mangled bulk transfer and an afternoon of debugging logic that was
    never actually loaded. The CRC now also stops a mangled chain from ever
    reaching the fabric.
    """
    t0 = time.time()
    ok, msg = cfgplane.load(p, bs.to_int(), FABRIC_CFG_W, start=True,
                            fast_first=not force_slow)
    dt = time.time() - t0
    if ok:
        if verbose:
            print(f"  {msg} in {dt:.2f}s")
        return True
    print(f"  FAIL  {msg}")
    return False


def go_live(p):
    cfgplane.ir(p, "BYPASS")


# --- verification ----------------------------------------------------------

def intest_sweep(p, vectors):
    """Apply each vector through the boundary and collect pad_o.

    INTEST is pipelined by one scan: what a scan captures answers the vector the
    previous scan committed. So prime with the first vector and read one late.
    """
    cfgplane.ir(p, "INTEST")
    vs = list(vectors)
    p.intest(vs[0])
    out = []
    for k in range(len(vs)):
        nxt = vs[(k + 1) % len(vs)]
        out.append(p.intest(nxt))
    return out


def verify(p, key, desc, fn, sweep):
    bs = fn().build()
    print(f"\n[{key}] {desc}")
    if not load_bitstream(p, bs):
        return False
    vs = list(sweep)
    got = intest_sweep(p, vs)
    bad = []
    for v, g in zip(vs, got):
        e = simulate(bs, v)
        if g != e:
            bad.append((v, g, e))
    if bad:
        print(f"  FAIL  {len(bad)} of {len(vs)} vectors wrong")
        for v, g, e in bad[:6]:
            print(f"        pad_i={v:06b}  pad_o={g:03b}  expected {e:03b}")
        return False
    print(f"  PASS  {len(vs)} vectors match the model")
    return True


# --- live view -------------------------------------------------------------

def watch(p, bs=None, label="", refresh=0.05):
    print(f"  {label or 'monitoring the fabric'}")
    print("  flip SW0 / SW1, press the buttons. Ctrl-C to stop.\n")
    cfgplane.ir(p, "SAMPLE")
    lines = 0
    try:
        while True:
            s = p.sample()
            i = s["i"]
            body = [
                "   pads in    SW1={} SW0={}   BTN3..0 = {:04b}   (pad_i = {:06b})".format(
                    (s["sw"] >> 1) & 1, s["sw"] & 1, s["btn"], i),
                "   pads out   LD2={}  LD1={}  LD0={}".format(
                    s["cout"], s["o5"], s["o"]),
            ]
            if bs is not None:
                try:
                    e = simulate(bs, i)
                    got = (s["cout"] << 2) | (s["o5"] << 1) | s["o"]
                    body.append("   model      {:03b}                     {}".format(
                        e, "match" if e == got else "MISMATCH"))
                except ValueError:
                    body.append("   model      (not a combinational design)")
            if lines:
                sys.stdout.write(f"{CSI}{lines}A")
            for ln in body:
                sys.stdout.write(f"{CSI}2K" + ln + "\n")
            sys.stdout.flush()
            lines = len(body)
            time.sleep(refresh)
    except KeyboardInterrupt:
        print("\n  stopped. The fabric is still configured and still live.")


# --- entry points ----------------------------------------------------------

def cmd_list():
    print("built-in designs:\n")
    for key, desc, fn, sweep in DESIGNS:
        print(f"  {key:9s} {desc}")
    print("\nload one with:  ./fpga.py --load <name>")
    return 0


def cmd_report(key):
    if key not in BY_KEY:
        sys.exit(f"unknown design '{key}'. try --list")
    desc, fn, sweep = BY_KEY[key]
    d = fn()
    bs = d.build()
    print(f"{key}: {desc}\n")
    print(d.report())
    print(f"\n  bitstream: {FABRIC_CFG_W} bits, {bin(bs.to_int()).count('1')} set")
    print("\n  truth table by the software model:")
    for v in sorted(set(list(BY_KEY[key][2])))[:16]:
        print(f"    pad_i={v:06b} -> pad_o={simulate(bs, v):03b}")
    return 0


def main():
    ap = argparse.ArgumentParser(description=__doc__,
                                 formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--list", action="store_true")
    ap.add_argument("--report", metavar="NAME")
    ap.add_argument("--load", metavar="NAME")
    ap.add_argument("--selftest", action="store_true")
    ap.add_argument("--watch", action="store_true")
    ap.add_argument("--bscan", action="store_true",
                    help="interactive boundary-scan control (INTEST/EXTEST/SAMPLE)")
    ap.add_argument("--lines", action="store_true",
                    help="in --bscan, type a key then Enter instead of using "
                         "single keystrokes (use if your terminal misbehaves)")
    ap.add_argument("--slow", action="store_true",
                    help="shift the config per-pulse instead of in bulk")
    ap.add_argument("--freq", type=int, default=100, help="TCK frequency in kHz")
    args = ap.parse_args()

    if args.list:
        return cmd_list()
    if args.report:
        return cmd_report(args.report)

    p = Probe(freq_khz=args.freq)
    print(f"probe: {p.info()}")
    idcode = p.read_idcode()
    if idcode != IDCODE_FABRIC:
        print(f"\n  IDCODE = 0x{idcode:08X}, expected 0x{IDCODE_FABRIC:08X}")
        print({0x1BEEF093: "  That is the bare TAP build.",
               0x2BEEF093: "  That is the single-CLB build - this tool needs the 4x4 fabric.",
               0x3BEEF093: "  That is the M0/M1 fabric with the old 4-bit TAP - rebuild hw/ (M3).",
               0x4BEEF093: "  That is the M2 configuration-plane test top, not the fabric.",
               0x5BEEF093: "  That is the M3 fabric (no user clock, 181-bit tiles) - rebuild hw/ (M4)."}
              .get(idcode, "  Run ./host/tap_probe.py to work out why."))
        print("  Build and program hw/ (top fpga4x4_top) first.")
        return 1
    print(f"IDCODE 0x{idcode:08X}  - 4x4 fabric is alive\n")

    if args.selftest:
        ok = all(verify(p, k, d, f, s) for k, d, f, s in DESIGNS)
        go_live(p)
        print("\n=== ALL DESIGNS PASSED ===" if ok else "\n=== SOME DESIGNS FAILED ===")
        return 0 if ok else 1

    if args.watch:
        watch(p)
        return 0

    if args.bscan and not args.load:
        bscan.run(p, lines_mode=args.lines)
        return 0

    key = args.load or "showcase"
    if key not in BY_KEY:
        sys.exit(f"unknown design '{key}'. try --list")
    desc, fn, sweep = BY_KEY[key]
    d = fn()
    bs = d.build()
    print(f"{key}: {desc}\n")
    print(d.report())
    print()
    if not load_bitstream(p, bs, force_slow=args.slow):
        return 1
    go_live(p)
    print()
    if args.bscan:
        bscan.run(p, model=lambda v: simulate(bs, v),
                  label=f"{key}: {desc}", lines_mode=args.lines)
    else:
        watch(p, bs, label=f"{key}: {desc}")
    return 0


if __name__ == "__main__":
    sys.exit(main())

