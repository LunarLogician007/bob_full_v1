...
def check_showcase_live(p, ctx):
    ...
    exp = simulate(bs, s["i"])
    return got == exp, f"pad_i={s['i']:06b} pads={got:03b} model={exp:03b}"
...
def _counter_value(capture_bits):
    """q0..q3 of the counter live in tiles (0,0),(1,0),(2,0),(3,0) = CAPTURE bits 0,4,8,12."""
    return sum(((capture_bits >> (4 * r)) & 1) << r for r in range(4))
...
def check_counter_step(p, ctx):
    """JTAG-stepped counter: exactly one count per TCK edge (21 edges per CAPTURE scan -> +5)."""
    import cfgplane
    import fpga
    from bitstream import FABRIC_CFG_W
    ...
    vals = [_counter_value(p.shift_dr(16, 0)) for _ in range(6)]
    diffs = [(b - a) % 16 for a, b in zip(vals, vals[1:])]
    cfgplane.user1(p, 0b100)                     # cin only: ce off
    cfgplane.ir(p, "CAPTURE")
    held = [_counter_value(p.shift_dr(16, 0)) for _ in range(3)]
    return (all(d == 5 for d in diffs) and len(set(held)) == 1,
            f"counts {vals} (steps {diffs}, want 5 each); ce off: {held} (want constant)")
...
def check_counter_run(p, ctx):
    ...
    cfgplane.ir(p, "CAPTURE")
    t0 = time.time()
    last = _counter_value(p.shift_dr(16, 0))
    counts = 0
    while time.time() - t0 < 3.0:
        v = _counter_value(p.shift_dr(16, 0))
