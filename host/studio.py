...
reimplements a stage; every route drives tools/bob/flow.py (synthesis and its
equivalence check, place and route, FASM, bitgen, the model check) or
host/cfgplane.py (program, readback), and reports what they return.
  ...
  GET  /api/browse              the designs you can open: designs/, examples/, and any .proj
  GET  /api/bits                the bitstreams in build/bit/, newest first
  POST /api/new                 scaffold designs/<name>/<name>.v from a template
...
def browse():
    """Everything openable, newest first within each group. designs/ is where your own
    work goes; examples/ ships with the project."""
    out = []
    for root, label in ((DESIGNS, "design"), (os.path.join(ROOT, "examples"), "example")):
...
def new_design(name):
    """designs/<name>/<name>.v from the template. -> its repo-relative path.

    A name that is not already a plain name is refused, not quietly rewritten: asking
    for "../../etc/x" and silently getting designs/x/x.v is worse than an error."""
    name = str(name).strip()
    ...
    rel = os.path.join("designs", name, f"{name}.v")
...
def examples():
    ...
    for f in sorted(os.listdir(EXAMPLES)):
        ...
        out.append({"name": name, "path": os.path.join("examples", f),
                    "lines": sum(1 for _ in open(os.path.join(EXAMPLES, f))),
