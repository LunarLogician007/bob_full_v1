...
| M23 | the grid sweep (`software/bob/gridsweep.py`: SLICEM slices were the wall), crossbar muxes in pairs sharing dual-output CFGLUT5 leaves with OR roots (`lxpair.v`: 152 → 80 per CLB), routing muxes as LUT6 + MUXF7/MUXF8 primitives; **12 × 11 = 132 CLBs = 528 LUTs**, W 36, fc_in 0.10; 24-bit chain count; IDCODE `0x0B023093` | code and simulation on branch `m23`; waiting for the Vivado build. yosys: 31,856 LUT + 10,560 CFGLUT5 + 33,550 FF. Checklist `docs/hwtest/M23.md`, sweep `docs/reports/M23/grid_sweep.md` |
...
### M23: 12 × 11 CLBs from two cheaper building blocks
...
holds one CLB's, and Vivado packed 3.37 per slice (84% of the SLICEMs at 81 CLBs). Two
changes move it:

- **Crossbar pairs** (`hw/src/clb/lxpair.v`). With I4 tied high a CFGLUT5 is two 4-input
  tables over one set of inputs (UG953: O5 = bits 15:0, O6 = bits 31:16). All 24 crossbar
  muxes of a CLB see the same 24 sources, so muxes 2p and 2p+1 share six leaves. An
  unselected leaf holds zeros, so the root is a fixed OR in a plain LUT6. That is 80
  CFGLUT5s per CLB instead of 152; `lut_expand.v` fills bits 31:16 from the odd select and
  15:0 from the even one. The bitstream is unchanged.
- **Routing muxes from primitives** (`hw/src/fabric/bob_mux.v`): LUT6 4:1 leaves on
  sel[1:0], MUXF7 on sel[2], MUXF8 on sel[3], a 16:1 mux per slice (UG474). Vivado made
  0.785 × the yosys count of M22's behavioural muxes; the primitives cost `hand_luts()`
  exactly (checked in yosys), about 30% fewer.
- **Chosen: 12 × 11 = 132 CLBs** (528 LUTs, 1.63× M22) at W 36, fc_in 0.10. fir16 needs
  34 tracks, every other example 20. Predicted 39.2k LUTs (74%), 84% of slices, 72% of
  SLICEMs at M22's packing. 12 × 12 would sit at 91%; 12 × 10 and 11 × 11 are the fallbacks.
...
- **Board check:** `xbar-pairs`, the snake on every pin with the other pins at const1.
