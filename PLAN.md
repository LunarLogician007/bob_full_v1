...
> Last updated 2026-09-26 (see `HANDOFF.md` for the live state): M0–M25 passed on the PYNQ-Z2 (git tags `m7`…`m25`); the board runs M25 and the hardware is locked. Whole-project report: `docs/project/REPORT.md`, interactive `project.html`; slides: `docs/presentation/slides.tex`.
...
**The device on the board at M7 (`ARCH_6X4`; today's M25 device is in §2 and `README.md`):**
...
The 8×8 profile (48 CLBs, 9400 bits, `0x9BEEF093`) is frozen in `release/M7_8x8/` (git tag m25), because its Vivado synthesis was too slow on the build machine.
...
| M22 | LUT contents and the crossbar in AMD CFGLUT5 (ZUMA-style), measured first (a CLB: 469 LUTs + 424 config FFs → 193 LUTs + 48 FFs, yosys); 9 × 9 = 81 CLBs = 324 LUTs; `lut_loader.v` shifts each written frame into its CLB's CFGLUT5s, `lut_expand.v` expands the compact selects, JPROGRAM sweeps zeros; nothing on the wire changes; IDCODE `0x0B022093` | **on the board 2026-09-24: 65/67**; the two failed fir16 live checks rerun 12/12 (one interactive rerun pending). Vivado WNS +0.172 ns, 38,184 LUTs (71.8%), SLICEM 70.6%. Checklist `docs/hwtest/M22.md`, spec `docs/bitstream-format.md` section 15 |
...
### M22: LUT contents in CFGLUT5, 9 × 9 CLBs

M21 filled 90.9% of the chip's slices: 36.4k LUTs and 36.3k flip-flops, 32.9k of them
configuration. Measured in yosys, one M21 CLB is 469 host LUTs plus 424 configuration
flip-flops; with each element's truth table and each crossbar mux in AMD CFGLUT5s (a LUT5
whose table is shifted in, UG953; ZUMA's trick, Brant & Lemieux FCCM 2012) it is 193 LUTs
(152 CFGLUT5) and 48 flip-flops. CFGLUT5 needs SLICEM (17,400 LUTs on the XC7Z020), so the
routing muxes stay flip-flop configured - in LUTRAM they would cap the fabric near 48 CLBs.
The user chose **9 × 9 = 81 CLBs, 324 LUTs** (SLICEM ~71%).

- **Nothing on the wire changes.** A CLB tile is frame aligned and starts with its
  L-frames (the truth tables, then the crossbar selects); `cfg_store.v` keeps no flip-flops
  for them. `lut_loader.v` copies each written frame on the falling TCK edge that writes it
  and shifts 32 TCK cycles into the owning CLB's CFGLUT5s; a shared `lut_expand.v` turns
  selects into tree contents. JPROGRAM sweeps zeros; JSTART waits for the loader.
- **Simulation:** `hw/tb/prims/CFGLUT5.v` (sim only, `sim/hwfiles.sh --sim`) resolves an
  unknown address when the candidate entries agree (as UNISIM) and shows contents between
  shifts, so a half-shifted table cannot spin a zero-delay loop. `tb_clb` loads the chip's
  way; `tb_bob` checks CFGLUT5 contents against an independent model and the sweep.
- **Board:** the M21 list plus `lutram-snake` (all 324 elements in one chain).
- **Found on the way:** two M21 mutants were stale (`carry-direct-cut` cut a column no test
  used and survived; `xbar-no-feedback` no longer applied), and `make -k mutate` never ran
  `mutate_frames.sh` after a failing suite. Fixed on `m21`; `MUTATE_ONLY` reruns named mutants.
...
wherever it likes. The finished build's report named the failing clock: TCK, the
instruction register through 6349 levels of empty fabric into the DSP JTAG register
(5463 ns against a 5000 ns half period); sysclk had met once relaxed. Raising the gap
would not have helped, and each doubling halves the default guest clock.
...
### M23: 10 × 10 CLBs from two cheaper building blocks

`software/bob/gridsweep.py` builds VPR's routing graph for every grid × fc_in × W where all
the examples route, prices each routing mux with its measured yosys cost and the M21/M22
Vivado calibration, and writes `docs/reports/M23/grid_sweep.md`. The wall at M22 was
**SLICEM slices**: the four CFGLUT5s of a SLICEM share a shift enable, so one SLICEM only
holds one CLB's, and Vivado packed 3.37 per slice (84% of the SLICEMs at 81 CLBs).

- **Crossbar OR roots** (`hw/src/clb/lxor.v`). An unselected CFGLUT5 leaf holds zeros, so a
  crossbar mux's root is a fixed OR in a plain LUT, not a table: 128 CFGLUT5 per CLB instead
  of 152. `lut_expand.v` fills the leaves from the same 5-bit selects.
- **Routing muxes from primitives** (`hw/src/fabric/bob_mux.v`): LUT6 4:1 leaves on
  sel[1:0], MUXF7 on sel[2], MUXF8 on sel[3], a 16:1 mux per slice (UG474). The primitives
  cost exactly `hand_luts()` (checked in yosys), about 30% fewer than Vivado made of M22's
  behavioural muxes.
- **Chosen: 10 × 10 = 100 CLBs** (400 LUTs, 1.23× M22) at W 36, fc_in 0.10. fir16 needs
  34 tracks, every other example 20. SLICEM slices 87% at M22's packing; 11 × 10 would be 96%.
- **What did not work:** sharing each leaf between two crossbar muxes via O5/O6 (80 CFGLUT5
  per CLB on paper, 12 × 11). Vivado maps a dual-output CFGLUT5 to SRL16E + SRLC32E, two
  LUT sites. The first M23 build hung placing LUTRAM ("Weighted LUTRAM utilization is more
  than 100").
- **Old 16-bit limits fixed:** `cfg_ctrl.v`'s chain count is 24 bits (CFG_CTRL reports the
  low 16; `len_err` is the full test); the L-bit mask is a frame → kind table
...
9. **Git:** commit as work progresses (never `docs/bob_full_v1_report.tex`, which the user edits); tag `mN` only after milestone N passes on the board. The tags replace the `release/mac_MN` (git tag m25) copies for freezing tools.
...
bob_full_v1/
  ...
  docs/
    ...
    reference/ wiring.md architecture.html fabric-audit.html report.tex bob_full_v1_report.tex (user's)
...
| Vivado synthesis of 48 CLBs (M7) | 30+ min and still running on the build machine | Board runs the 16-CLB profile; 8×8 frozen in `release/M7_8x8/` (git tag m25) |
...
| Parallel work on hw/ and tools (M4, M7) | editing while the previous milestone was on the board; M6 tools had to be restored | freeze before starting N+1: git tag `mN` (earlier: `release/hw_MN` (git tag m25), `release/mac_MN` (git tag m25)) |
...
- **Retired:** `fabric.v`, `tile.v`, `mux_bank.v`, `fpga4x4.v`, `fpga4x4_top.v`, `bram_tile.v`, `dsp_tile.v`, `tb_fpga4x4.v` (frozen in `release/hw_M6/` (git tag m25)). `clb_pkg.sv` now has CLB constants only. `bscan.py` (interactive, 9-cell) is not ported.
...
- **Candidates**, to be measured on the M7 reports before choosing:
  ...
  4. **Larger grid only:** build `release/M7_8x8` (git tag m25) on a faster machine as it is.
