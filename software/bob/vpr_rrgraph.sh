...
#
#   software/bob/vpr_rrgraph.sh          (make rrgraph runs this between two device.py calls)
#
# For each LUT size: VPR loads software/bob/arch/bob_k<K>.xml, packs, places and
...
#   software/bob/arch/bob_k<K>_rr.xml.gz     the graph the fabric RTL is generated from
#   software/bob/arch/bob_k<K>_rr.stamp      sha256 of the arch it came from, image, command
#   software/bob/arch/bob_k<K>_vpr.txt       the tail of VPR's log (route result)
set -euo pipefail
cd "$(dirname "$0")/arch"
...
for K in 6 4; do
    ARCH=bob_k$K.xml
    [[ -f $ARCH ]] || { echo "missing $ARCH: run software/bob/device.py --arch-only"; exit 1; }
