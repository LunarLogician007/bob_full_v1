...
writes, as frames of FRAME_WORDS 32-bit words, carried in type-1/type-2 packets:
  ...
  partial_streams(old, new)
                         M14: (freeze, frames) - freeze: sync, RCRC, IDCODE, AGHIGH;
                         then (after STAT shows GHIGH_B = 0) frames: WCFG, FAR + FDRI
                         per run of changed frames, CRC, LFRM (releases), DESYNC
...
STAT_BITS = {"CRC_ERROR": 0, "GTS_CFG_B": 5, "GWE": 6, "GHIGH_B": 7, "INIT_COMPLETE": 11, "INIT_B": 12, "DONE": 14,
...
def far(column, minor=0, block_type=0, row=0):
    return (block_type << 23) | (row << 17) | (column << 7) | minor


def bram_far(b, frame=0):
    """M15: FAR of BRAM b's content frame `frame` (addresses 4*frame .. 4*frame+3)"""
    return far(b, frame & 0x7F, block_type=1, row=frame >> 7)


def bram_words(words):
    """a BRAM's contents as FDRI data: all 1024 addresses, {14'b0, data[17:0]}"""
    full = list(words) + [0] * (BRAM_WORDS - len(words))
    return [w & 0x3FFFF for w in full[:BRAM_WORDS]]
...
def changed_frames(old, new):
    """frame indices whose FB bits differ between two memory words"""
    mask = (1 << FB) - 1
    return [f for f in range(NFRAMES) if ((old ^ new) >> (FB * f)) & mask]


def _far_of(fidx):
    for c in FRAMES["columns"]:
        if c["base"] <= fidx < c["base"] + c["count"]:
            return far(c["far_col"], fidx - c["base"])
    raise ValueError(f"frame {fidx} is in no FAR column")


def partial_streams(old, new, idcode=None, crc_override=None):
    """M14: partial reconfiguration of a running design from memory `old` to `new`.

    Returns (freeze, frames, nframes). Send `freeze`, check STAT GHIGH_B = 0 (the fabric's
    user clock is held), then send `frames`: WCFG, one FAR + FDRI per run of consecutive
    changed frames (FAR auto-increments within a run), the CRC over everything since
    RCRC, and LFRM, which releases the freeze only if that CRC matched."""
    idcode = device_idcode() if idcode is None else idcode
...
def bram_readback_stream(b, first=0, n=BRAM_FRAMES):
    ...
    return ([DUMMY, SYNC, NOP] + write("CMD", CMD["RCFG"]) + write("FAR", bram_far(b, first)) +
            [type1(OP_READ, REG["FDRO"], 0), type2(OP_READ, FW * n)] + write("CMD", CMD["DESYNC"]) + [NOP])
...
def to_jtag(words):
    ...
    if not words:
        return 0, 0
    bits = "".join(format(w & 0xFFFFFFFF, "032b") for w in words)      # char i = bit i on TDI
    return len(bits), int(bits[::-1], 2)


def from_jtag(value, nwords):
    n = 32 * nwords
    bits = format(value & ((1 << n) - 1), f"0{n}b")[::-1] if n else ""   # char i = bit i out
    return [int(bits[32 * i:32 * i + 32], 2) for i in range(nwords)]
...
class Controller:
    ...
    def reset(self):
        ...
        self.flags = dict(wcfg=0, rcfg=0, lfrm=0, id_ok=0, crc_ok=0, data_seen=0, crc_err=0, id_err=0,
                          pkt_err=0, wr_err=0, start_ok=0, freeze=0)
        self.buf = []
        self.rd = (0, 0)
        self.rwidx = 0
    ...
    def bfar_valid(self):
        """M15: FAR block type 1 (BRAM contents)"""
        f = self.far
        return f >> 26 == 0 and (f >> 23) & 7 == 1 and ((f >> 18) & 0x1F) == 0 and ((f >> 7) & 0x3FF) < NBRAM

    def bfar(self):
        return (self.far >> 7) & 0x3FF, (((self.far >> 17) & 1) << 7) | (self.far & 0x7F)
    ...
    def far_next(self):
        if self.bfar_valid():
            b, n = self.bfar()
            if n != BRAM_FRAMES - 1:
                return bram_far(b, n + 1)
            return bram_far(b + 1) if b + 1 < NBRAM else far(1023)
        cols = self._cols()
        col, minor = (self.far >> 7) & 0x3FF, self.far & 0x7F
    ...
    def frozen(self):
        return bool(self.flags["freeze"] and self.ack)
    ...
    def shift_in(self, nbits, value):
        bits = format(value & ((1 << nbits) - 1), f"0{nbits}b")[::-1] if nbits else ""
        for ch in bits:
            bit = ch == "1"
    ...
    def _word(self, w):
        ...
        if reg == REG["CRC"]:
            ...
            if w == CMD["WCFG"]:
                ...
                f["lfrm"] = 1
                if f["freeze"]:
                    if f["crc_ok"] and not self.errors():
                        f["freeze"] = 0
                    else:
                        self._err("wr_err")
            elif w == CMD["AGHIGH"]:
            ...
            f["data_seen"] = 1
            if self.bfar_valid():
                if f["wcfg"] and f["id_ok"] and not self.gwe and not self.errors():
                    self.buf.append(w)
                    if len(self.buf) == FW:
                        b, n = self.bfar()
                        for k, x in enumerate(self.buf):
                            self.brams[b][4 * n + k] = x & 0x3FFFF
                        self.far = self.far_next()
                        self.buf = []
                else:
                    self._err("wr_err")
            elif f["wcfg"] and f["id_ok"] and (not self.gwe or self.frozen()) and self.far_valid() \
                    and not self.errors():
                self.buf.append(w)
                if len(self.buf) == FW:
    ...
    def read_word(self, gsr=1, gts=1, gwe=0, done=0):
        ...
        self.rd = (reg, cnt - 1)
        if reg == REG["FDRO"] and self.bfar_valid():
            b, n = self.bfar()
            w = self.brams[b][4 * n + self.rwidx] if (self.flags["rcfg"] and not gwe) else 0
            if self.flags["rcfg"]:
                if self.rwidx == FW - 1:
                    self.rwidx = 0
                    self.far = self.far_next()
    ...
    def stat(self, gsr=1, gts=1, gwe=0, done=0):
        f = self.flags
        v = f["crc_err"] | (int(not gts) << 5) | (gwe << 6) | (int(not self.frozen()) << 7) | (1 << 11)
        v |= int(not self.errors()) << 12
