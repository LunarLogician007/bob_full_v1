...
synth.py - yosys synthesis of a Verilog design onto the bob cell library (M8).
  ...
  software/bob/synth.py work/examples/counter/counter.v [more.v ...] [--top counter] [--out DIR]

The script is yosys' synth_xilinx flow (`yosys -p "help synth_xilinx"`) with
bob's cells substituted (software/bob/synth/):
...
Outputs in DIR (default build/synth/<top>):
  <top>.json     the netlist the placer reads (software/bob/place.py)
