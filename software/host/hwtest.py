...
an earlier one:
  ...
  selftest  every design in software/host/designs.py loads, reads back and matches the
...
ROOT = buildcfg.ROOT
sys.path.insert(0, os.path.join(ROOT, "software", "bob"))     # chainbits, model
...
def check_ram_readback(p, ctx):
    ...
    path, word, contents, tr, _work = cli.build([os.path.join(ROOT, "work", "examples", "ram", "ram.v")], "ram", None,
...
MILESTONE = {
    ...
    # M1: no RTL change. selftest (regression) now runs on software/host/bitstream.py
    ...
    # (software/bob/pnr/) instead of VPR, through the M10 golden check, then two live.
...
def check_partial_guest(p, ctx):
    ...
    for name in ("gates", "gates_swapped"):
        import vpr_run
        top, pcf = vpr_run.VARIANTS.get(name, (name, None))
        paths[name] = cli.build([os.path.join(ROOT, "work", "examples", top, f"{top}.v")], top, pcf,
