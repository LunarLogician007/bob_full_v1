...
sim/gen_vectors.py turns these into simulation vectors and software/host/fpga.py loads
...
def d_far_corner():
    ...
    d = Design()
    d.output(0, d.lut(FAR_X, FAR_Y, LUT.or2(), [d.input(0), d.input(1)]))
    return d
...
def d_cross():
    ...
    inverted at the North-East, XORed with SW0 at the South-East."""
    d = Design()
    a, b = d.input(0), d.input(1)
    g1 = d.lut(CLB_COLS[1], FAR_Y, LUT.and2(), [a, b])
    g2 = d.lut(FAR_X, FAR_Y, LUT.inv(0), [g1])
    g3 = d.lut(FAR_X, CLB_ROWS[1], LUT.xor2(), [g2, a])
    d.output(0, g3)
    d.output(1, g1)
    d.output(2, g2)
    return d
...
_ALL = [
    ...
    ("corner",  "OR in the far North-East CLB, away from the input pads",     d_far_corner, range(4),  2),
...
def d_gsr_probe():
    ...
    it clocks to 0. Used by hw/tb/tb_bob.v and software/host/hwtest.py.
    """
    d = Design()
...
# --- M4: sequential designs (checked with software/bob/model.py) ------------------------
