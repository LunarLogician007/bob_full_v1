...
//
// Frame writes need WCFG, a matched IDCODE, GWE = 0, no error and a valid FAR.
...
module cfg_frames #(
    ...
    assign stat = {rcfg, start_ok, gsr, crc_ok, wcfg, synced, wr_err, pkt_err,
                   VERSION,
                   id_err, done, 1'b0, ~any_error, 1'b1, 3'b000, 1'b0, gwe, ~gts, 4'b0000, crc_err};
    ...
    always @(posedge tck) begin
        ...
        if (jprogram) begin
            ...
            if (sel_out && (dr_capture || (dr_shift && ocnt == 5'd31))) begin
                ...
                if (rd_cnt != 27'd0) begin
                    rd_cnt <= rd_cnt - 27'd1;
                    if (rd_reg == R_FDRO && rcfg && far_valid) begin
