...
Where it stands (2026-09-24): **M0–M22 passed on the board.** The board runs M22: LUT contents and crossbar in CFGLUT5, 9 × 9 = 81 CLBs, IDCODE `0x0B022093`. **M23** (10 × 10 = 100 CLBs = 400 LUTs: crossbar roots as a plain OR, routing muxes from LUT6/MUXF7/MUXF8, 24-bit chain count; never use a CFGLUT5's O5, Vivado makes it two LUT sites) is on branch `m23` in the worktree `../bob_full_v1_m23`, waiting for its Vivado build (`docs/hwtest/M23.md`). Vivado must still run without the XDC in synthesis and with no `keep_hierarchy`.
