...
Where it stands: **M0–M16 all passed on the PYNQ-Z2.** M16 is the 10 × 10 grid (100 CLBs, 145 frames, 18 560 bits, IDCODE `0xFBEEF093`): 20 498 LUTs (38.5%), 21 511 FFs, WNS +0.667 ns. Since M16 there is **bob studio** (`software/host/studio.py --probe fake`), an EDA tool over the same flow; its engine is `software/bob/flow.py` and it can drive the board in software (`software/host/fakeboard.py`). The device table in the documents is generated (`software/bob/devtable.py`), not typed.
