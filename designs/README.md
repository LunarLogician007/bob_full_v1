...
Your own designs live here; `examples/` ships with the project.

bob studio's **Sources → New design** creates `designs/<name>/<name>.v` from a template,
and **Pin Planner → write .pcf** puts a pin file beside it. From the command line:

```sh
./bob build designs/mything/mything.v
./bob build designs/mything/mything.v --pcf designs/mything/mything.pcf
